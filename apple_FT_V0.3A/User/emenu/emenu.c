...
s32 emenu_display(DevLcdNode *lcd)
{
	...
	debug(LOG_DEBUG,"emenu display:%s\r\n", menu_ctrl.fa->cha + lang);
	...
	switch(key)
	{
		...
		case EMENU_KEYU_ENTER://enter
			...
			if(menu_ctrl.sel->l == (menu_ctrl.fa->l)+1)
			{
				...
				{
					debug(LOG_DEBUG, "sel:%s\r\n", menu_ctrl.sel->cha);
					debug(LOG_DEBUG, "dis:%s\r\n", menu_ctrl.dis->cha);
 ...
 *@retval:     
 */
s32 emenu_run( MENU *p, u16 len, FontType font, u8 spaced)
{
	u8 disflag = 1;
