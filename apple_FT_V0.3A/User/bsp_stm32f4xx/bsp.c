...
*		V1.1    2013-06-20  plxc   规范注释，添加必要说明
*
...
void bsp_Init(void)
{
	...
	*/
    ...
    /* 直接调用 taget->IROM1内设置的数据 */
    extern int Image$$ER_IROM1$$Base; /* MDK 里面内置的特殊变量: taget->IROM1内设置的数据 */
    NVIC_SetVectorTable(NVIC_VectTab_FLASH, (uint32_t)&Image$$ER_IROM1$$Base);
    /* 检测系统是否从独立看门狗复位中恢复 */
    dut_param_init();
	key_init();
	bsp_InitUart(); 	/* 初始化串口 */
...
//    volt_out_init();   /* 初始化4.2V OUT，关掉输出 */
//	short_100R_init(); /*初始化100R采样电阻，不接入100R*/
    bsp_InitADC();
   //  
   bsp_DelayMS(10);
