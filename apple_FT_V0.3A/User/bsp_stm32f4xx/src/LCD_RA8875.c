...
*	修改记录 :
*		版本号  日期       作者    说明
*		V1.0    2012-08-07 plxc  发布首版
*		V1.1    2012-10-23 plxc  解决绘制矩形函数的BUG（高度宽度计算错误）；增加SPI-4接口模式；
*								   解决几个绘制2D图形的函数判忙条件不真实的问题。
*								   触摸校准增加4点校准
*		V1.2    2012-11-07 plxc  解决4点校准的BUG
*		V1.3    2012-12-12 plxc  解决7寸屏，长线出现麻点的问题
*		V1.4    2013-05-04 plxc  增加进入绘图模式的函数，用于摄像头直接写屏 RA8875_StartDirectDraw
*									RA8875_GetDispMemAddr, RA8875_QuitDirectDraw
*       V1.5    2013-06-12 plxc  为避免IAR告警，去掉 s_WinX，s_WinY, s_WinHeight,s_WinWidth前的__IO修饰
*		V1.6    2013-06-26 plxc  增加函数  RA8875_CtrlGPO() 控制KOUT0-KOUT3 口线的状态
*		V1.7    2013-06-26 plxc  增加函数  RA8875_CtrlGPO() 进入低功耗
*		V1.8    2013-06-29 plxc  修改 RA8875_GetPixel() 函数BUG，需要读2次。并且设置0x40寄存器为读光标不自增
*		V1.9	2013-07-24 plxc  修改 RA8875_DrawLine() 函数, 当起始点和终止点相同时，可能死机.增加特殊处理
*		V2.0	2013-09-22 plxc  解决 RA8875硬件2D绘图的BUG，修改若干函数，解决触摸冲突问题。
*		V2.1    2013-09-26 plxc  将MCU接口相关的函数分离到 bsp_ra8875_port.c 文件。
*		V2.2    2014-01-31 plxc  增加 RA8875_DispBmpInFlash() 函数,显示RA8875外挂串行Flash中的BMP位图
*
...
void RA8875_InitHard(void)
{
	...
	【备注】这是普林芯驰为了便于统一测试例程，在LCD模块上做的硬件标识。在做产品时，不必自动识别。
	*/
	...
	/* 设置TFT面板的 DISP  引脚为高，使能面板. 普林芯驰TFT模块的DISP引脚连接到RA8875芯片的GP0X脚 */
	...
	uint32_t _uiFlashAddr)
{
	...
	*/
	...
	/* 05H  REG[05h] Serial Flash/ROM Configuration Register (SROC)
		7	Serial Flash/ROM I/F # 选择
				0:选择Serial Flash/ROM 0 接口。[普林芯驰RA8875驱动板图库芯片接在 1 接口]
	...
	*/
	...
	/*
		...
		普林芯驰TFT驱动板。4.3寸系统频率为 68MHz； 7寸系统频率为 81.25MHz。
...
void RA8875_DispStr(uint16_t _usX, uint16_t _usY, char *_ptr)
{
	/*
		...
		普林芯驰RA8875驱动板集成的字库芯片为 集通字库芯片_GT23l32S4W
	...
	*/
	...
	/* 05H  REG[05h] Serial Flash/ROM Configuration Register (SROC)
		7	Serial Flash/ROM I/F # 选择
				0:选择Serial Flash/ROM 0 接口。[普林芯驰RA8875驱动板字库芯片接在 0 接口]
				1:选择Serial Flash/ROM 1 接口。
		6	Serial Flash/ROM 寻址模式
				0: 24 位寻址模式。
				此位必须设为0。
	...
	else
	{
		...
		/* 	REG[8Ah] PWM1 Control Register (P1CR)
			...
			Bit3-0  PWM1 电路的频率来源选择PWM_CLK	【不是PWM输出频率】
				...
				普林芯驰的4.3寸(480*272)模块，SYS_CLK =  68.75MHz
				普林芯驰的7.0寸(800*480)模块，SYS_CLK =  81.25MHz
