...
static const X_GPIO_T s_gpio_list[HARD_KEY_NUM] = {
	{GPIOE, GPIO_Pin_7, 0},     /* up     */
	{GPIOE, GPIO_Pin_8, 0},     /* down   */
	{GPIOE, GPIO_Pin_9, 0},     /* set    */
    {GPIOE, GPIO_Pin_10, 0},    /* esc    */
    {GPIOE, GPIO_Pin_12, 0},    /* 机台CMD   */
};
...
static void bsp_InitKeyVar(void)
{
    ...
    //s_tBtn[KEY_COUNT-1].State = 1; //机台通讯默认高电平，发送cmd是100ms低电平脉冲
...
{
	s_tKey.Buf[s_tKey._Write] = _KeyCode;

	if (++s_tKey._Write >= KEY_FIFO_SIZE)
	{
		s_tKey._Write = 0;
	}
}
...
uint8_t bsp_GetKey(void)
{
	...
	if (s_tKey._Read == s_tKey._Write)
	{
		return KEY_NONE;
	}
	else
	{
		ret = s_tKey.Buf[s_tKey._Read];

		if (++s_tKey._Read >= KEY_FIFO_SIZE)
		{
			s_tKey._Read = 0;
		}
		return ret;
