...
*		V1.1    2013-06-09 plxc  FiFo结构增加TxCount成员变量，方便判断缓冲区满; 增加 清FiFo的函数
*
...
*********************************************************************************************************
*/
void USART2_SendBefor(void)
{
	//RS485_TX_EN();	/* 切换RS485收发芯片为发送模式 */
    //USART_HalfDuplexCmd(USART2,DISABLE);
//    GPIO_InitTypeDef GPIO_InitStructure={0};
    
//    GPIO_InitStructure.GPIO_OType = GPIO_OType_OD;	/* 输出类型为推挽 */
//    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;	/* 内部上拉电阻使能 */
...
static void UartVarInit(void)
{
	...
	g_tUart2.SendBefor = 0;//USART2_SendBefor;						/* 发送数据前的回调函数 */
	g_tUart2.SendOver = 0;//USART2_SendOver;						/* 发送完毕后的回调函数 */
...
static void InitHardUart(void)
{
    ...
    //USART_ITConfig(USART1, USART_IT_ERR, ENABLE);	/* 使能错误接收中断 */
	/*
	...
	*/
	...
	/* 第1步： 配置GPIO */
		...
		/* 将 PA2 映射为 USART2_TX. 在FT_F4板中，PA2 管脚用于以太网 */
	...
	/* 第2步： 配置串口硬件参数 */
	USART_InitStructure.USART_BaudRate = dut.asu_baud;	/* 波特率 */
	USART_InitStructure.USART_WordLength = USART_WordLength_8b;
	USART_InitStructure.USART_StopBits = USART_StopBits_1;
	USART_InitStructure.USART_Parity = USART_Parity_No ;
	USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;
	USART_InitStructure.USART_Mode = USART_Mode_Rx | USART_Mode_Tx;		/* 选择接收模式 和发送模式 */
	USART_Init(USART2, &USART_InitStructure);
    //USART_HalfDuplexCmd(USART2,ENABLE);
//    CLEAR_BIT(USART2->CR2,(USART_CR2_LINEN|USART_CR2_CLKEN));
//    CLEAR_BIT(USART2->CR3,(USART_CR3_IREN |USART_CR3_SCEN));
	USART_ITConfig(USART2, USART_IT_RXNE, ENABLE);	/* 使能接收中断 */
    //USART_ITConfig(USART2, USART_IT_ERR, ENABLE);	/* 使能错误接收中断 */
	/*
		USART_ITConfig(USART1, USART_IT_TXE, ENABLE);
		注意: 不要在此处打开发送中断
		发送中断使能在SendUart()函数打开
...
static void UartIRQ(UART_T *_pUart)
{
    ...
    if (USART_GetITStatus(_pUart->uart, USART_FLAG_ORE) != RESET) //过载，数据溢出（干扰数据错乱）
    {
        uint16_t temp = USART_ReceiveData(_pUart->uart); //取出扔掉
